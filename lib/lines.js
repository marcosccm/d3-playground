...
(function() {
  ...
  $(function() {
    ...
    path = svg.append('g').attr('clip-path', 'url(#clip)').append('path').data([data]).attr('class', 'line').attr('d', line);
    tick = function() {
      data.push(random());
      path.attr('d', line).attr('transform', null).transition().duration(1000).ease('linear').attr('transform', "translate(" + (x(-1)) + ")").each('end', tick);
      return data.shift();
    };
    return tick();
  });
